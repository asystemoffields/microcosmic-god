...
Collaboration is similarly counted by context (`expedition`, `tool_use`, `build`, `causal_step`, `causal_unlock`, and failure variants). Promoted collaboration records include helper ids, support magnitude, focus affordance, and place, so later story tools can identify genuinely group-enabled events without turning cooperation into a hidden objective.
