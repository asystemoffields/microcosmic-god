# Controller adaptability — can anything remarkable ever develop here?

*2026-06-11. Prompted by Alex: "if they don't have — genuinely — the ability to develop,
...
## What can develop today (audit of controller.py / params.py / optimization.py)

| Dimension | Adaptable? | Mechanism / bound |
|---|---|---|
| Hidden size | **Yes (capacity only)** | `neural_budget` params field, legal range 0–512, init 4–13; `resize_hidden` is function-preserving; upkeep cost scales with it |
| Episodic memory capacity | Yes | params field, 0–32 slots |
| Plasticity *rates* | Yes | scalar params fields (learning rate, trace decay, etc.) |
| All weights | Yes | uniform gaussian on every entry at spawning |
| Topology (depth, modules, wiring) | **No** | exactly one dense recurrent layer, forever |
| Attention head | **No gain path** | inherited if present; a line without one can never acquire it; form fixed (H×72, all-or-nothing) |
| Learning-rule *form* | No | fixed family; only its scalars develop |
| Prediction heads | No | fixed set |
| Perturbation granularity | No | one global scale for all weights; template combination picks ONE parent's network whole |
...
of extra capacity arrives only after lifetime learning fills it. That is a quality
...
ticks, max ever reached by any line, persistence curves conditioned on budget. If no
line ever crosses ~16, growth is being strangled, and the economics fix is justified
...
and spawning up 0.62 -> 1.18 child (+90%), while leaving the small-
capacity baseline undistorted (70/75% reach-150, ~1.95 child, both arms).
Large (49-128) persists under subsidy (67% vs 0% reach-150) but did not yet
spawn; random-init giants (>128) never establish in either arm - the
...
Gate (scale-aware): after the fix, lines must *explore* budget >20 within a standard
run. They don't have to keep it — we're testing that the valley is crossable, not that
big is better.

### Phase 2 — typed modular params (the real unlock; aligns with the v3 controller)
...
typed observation tokens (self / fields / objects / agents / consequences)
  ...
  -> K recurrent core blocks, params-owned wiring mask between them
  -> heads (action, prediction, optional per-block attention / episodic)
```

Structural params: K, per-block module flags, wiring mask, per-block plasticity scalars.
Structural perturbation operators, in order of importance:

1. **Duplicate-and-diverge:** copy a block (weights included), let the copy drift.
   Adaptation's proven route to innovation, and trivially dimension-safe with typed
   interfaces.
2. **Add-block, neutrally:** new block enters with its output gate near zero —
   function-preserving, so structural perturbation is *persistable* (Net2Net-style). The
   single biggest historical blocker for dramatic neuroevolution is that structural
   perturbations are lethal; neutrality removes the lethality.
...
identified — the same redesign serves transfer and adaptability at once.

### Phase 3 — adaptable plasticity
Per-block learning rates and trace decays (params vector, not one scalar), plus a
...
example: if, after the economics fix and 4× standard run length, no structurally-perturbed
line ever out-persists the fixed-shape pool, the modular params is not earning its
