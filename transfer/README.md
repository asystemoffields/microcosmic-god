...
A test rig for evaluating whether controllers developed in microcosmic-god have transferable representations — does the cognition that emerged in the alife substrate generalize to a different task it has never seen?
...
- `sample_controllers/` — committed controller checkpoints from the 30-min seed-1 run for quick experimentation:
...
## What we found (10 seeds, 150 ES cycles, learner_champion_hidden14)

```
Direct (no controller)     : +0.084 ± 0.252
Random-init controller     : +0.004 ± 0.400
Permuted trained controller: -0.227 ± 0.197
Trained controller         : -0.020 ± 0.360

trained − permuted     = +0.207   structure matters
trained − random_controller = -0.024   no transfer beyond random
trained − direct       = -0.104   controller hinders slightly
```
...
Drop any `controller_*.json` from `runs/<your-run>/checkpoints/` into `sample_controllers/` and update `SAMPLE_CONTROLLER` in the experiment cell. The adapter sizes itself to the controller's dimensions.

## Architecture

The controller is **frozen** during the test — only the input projection (`W_in`: 4 → input_size) and output projection (`W_out`: output_size → 3) train. The controller's `forward()` and attention head run in pure numpy, mirroring `microcosmic_god/controller.py`. Attention noise is set to zero for deterministic evaluation.
