...
**Question** (docs/TRANSFER_RUNWAY.md): does a controller evolved in this sandbox carry
...
weight distribution, structure destroyed), all sharing the champion's genome and
architecture, lifetime learning ON for everyone. Cohorts of 16 injected as founders into
12 held-out worlds (places=36), reproduction frozen, 500 ticks, paired by world seed.
...
|---|---|---|---|---|---|
| learner, in-dist | **+0.52** (t 21, w 100%) | +91 (t 31) | +38.0 (t 15) | −27 (t −10) | survivor-specialist; zero tool use |
| learner, OOD | **+0.33** (t 9, w 100%) | +85 (t 18) | +31.5 (t 11) | −24 (t −8) | transfer survives the harshness shift |
| overall, in-dist | **+0.43** (t 12, w 100%) | +99 (t 16) | +33.8 (t 18) | **+211** (t 17) | the generalist: survival AND tools |
| tool, in-dist | −0.05 (t −1.2, ns) | −24 (t −4) | +25.3 (t 8) | **+369** (t 61) | pure specialist; no survival edge |
...
1. **Within-substrate transfer is real.** Every champion except the tool specialist
   carries a survival advantage into novel worlds — winning all 12 paired worlds in
   every survival sweep. This is the result the Catch test could not deliver: with the
   native observation/action schema preserved, evolved structure beats random
   decisively.
2. **Structure, not conditioning.** Trained beats *permuted* nearly everywhere
   ...
   decomposition: weight *statistics* buy metabolic efficiency (permuted recovers most
   of the energy advantage), but *structure* buys staying alive and all behavior.
3. **Transfer is champion-shaped.** The learner transfers survival and suppresses
   exploration; the tool champion transfers prolific tool use (369 vs 0.6) at a small
   survival cost; the overall champion transfers both; the v2 (episodic-memory,
...
- **Strategy or stasis?** The learner survives with near-zero tool/causal activity. Needs
...
- Champions evolved at hidden 5–14 — capacity exploration is absent in the substrate's
  history (docs/CONTROLLER_EVOLVABILITY.md); what richer-bodied evolution transfers is
...
runs are not bit-reproducible across invocations, which the paired multi-seed design
