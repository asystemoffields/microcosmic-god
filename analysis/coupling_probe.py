...
Usage:
  python analysis/coupling_probe.py --checkpoint runs/.../brain_t..._founder.json
  python analysis/coupling_probe.py --checkpoints-dir runs/.../checkpoints --json out.jsonl

With --checkpoints-dir, every brain_*.json is probed and rows are sorted by the
...
from microcosmic_god.brain import TinyController  # noqa: E402
from microcosmic_god.modular import ModularController  # noqa: E402
from microcosmic_god.organisms import ACTIONS  # noqa: E402


def _load_controller(path: Path):
    data = json.loads(path.read_text())
    controller_dict = data.get("brain", data)
...
def main() -> None:
    ...
    paths = [args.checkpoint] if args.checkpoint else sorted(args.checkpoints_dir.glob("brain_*.json"))
