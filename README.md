...
| --- | --- | --- |
| `smoke` | Validate the engine in seconds | 12 places, 118 initial individuals, 300 ticks, 20s wall cap |
| `minute` | Local exploratory run | 36 places, 490 initial individuals, 8,000 ticks, 300s wall cap |
| `long` | Overnight or day-scale local run | 96 places, 3,000 initial individuals, 1,000,000 ticks, 24h wall cap |
| `modal` | Cloud/remote scaling target | 256 places, 5,800 initial individuals, 10,000,000 ticks, 3d wall cap |
...
Inactive agents lose their live ANN by default. The simulator only saves controllers when selected by checkpoint policy:
...
Every new mechanic should be governed by reusable laws rather than special-case objectives. Tool and environment mechanics should compose through properties like hardness, resistance, containment, traversal, insulation, conductivity, and learned skill.
